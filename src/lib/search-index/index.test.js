import { SearchIndex, SearchIndexCreationError, SearchIndexDocumentError } from ".";
...
describe("document update tests", () => {
    ...
    it("correctly updates documents already added to the index", () => {
        ...
        const asArrayBecauseItIsntForSomeReason = JSON.stringify(documentStore()["ref"])
    ...
    it("errors when adding a non-string field", () => {
        expect(() => {
            index.setReference("ref", "valid field", {not: "valid"}, "another valid field")
        })
    })
});
