...
export class StorageRegistry {
    ...
    constructor(storageKey) {
        if (!storageKey || typeof storageKey !== "string") {
            throw new RegistryCreationError(`StorageRegistry must be created with a non-null string (got '${storageKey}')`)
        }
    ...
    setKeyString(key, str) {
        if (key === '') {
            throw new RegistryStoreError(`Cannot set empty string ('') as storage key`)
        }
        if (typeof str !== "string" && typeof str !== "number") {
            if (typeof str === "symbol") {
                str = "Symbol()"
            }
            throw new RegistryStoreError(`Cannot set storage key '${key}' to non-string/number '${str}'`)
        }
        if (str.includes(SEPARATORS.pair) || str.includes(SEPARATORS.item)) {
            throw new RegistryStoreError(`Registry cannot store strings with reserved substrings '${SEPARATORS.pair}' or '${SEPARATORS.item}'`)
        }
