...
class TextNode extends React.Component {
    ...
    updateTextSize() {
        ...
        // height is also used to allow wrapping text
    ...
    _updateTextSizeNoWrap(maxWidth, maxHeight) {
        ...
        // this updates in increments of 144, 29, 6, and 1 as a searching optimization
    ...
    _updateTextSizeWrap(maxWidth, maxHeight) {
        ...
        // since wrapping text trades width for height, sometimes (when the
        // width is greater than the height) an element will wrap text without
        // trading all the width it can; therefore, to maximize height, we just
        // go another step and see if the width changes, and only stop when it
        // does
...
// (this works fine in small groups, but don't put more than a few of these in
// your app; if you need large numbers, use the faster StaticTextNode instead)
...
// a TextNode that only works with unchanging text, however it only needs to
// calculate an optimal font factor once (or twice...)

// (dynamic wrapping with this text node is disabled; to wrap text, you will
// need to manually insert newline characters)
    ...
    _onResize() {
        ...
        // in case lastResizeMode has not yet been initialized...
            ...
            if (Math.abs(heightDiff) < Math.abs(widthDiff)) {
                ...
                this._fitHeightRatio =
                    (this._fitWidthRatio * clientWidth) / clientHeight
