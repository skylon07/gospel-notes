...
const ERROR = {
    ...
    typeError(str) {
        throw new TypeError(
            `Custom hook useClassName() failed to process; ${str}`
        )
    },
}
...
const WARN = {
    extraOption(str) {
        console.warn(
            `Custom hook useClassName() got an extra option "${str}"; Did you mean to do this?`
        )
    },
}
    ...
    const tracker = {
        validate: (value) => {
            ...
            if (typeof value !== "string") {
                ERROR.internal(
                    `Processor tried to validate "${optionName}" with an invalid non-string value: ${value}`
                )
            }
            classString = value
        },
...
function joinValidatedStrings(trackerDict) {
    ...
    for (const trackerName in trackerDict) {
        ...
        if (!tracker.isValidated) {
            ...
            // joining process)
            ERROR.internal(
                `Option "${trackerName}" was never marked as validated`
            )
        }
...
class Processor {
    ...
    static processChoices(options, state) {
        ...
        const choiceStrings = option.reduce((lastResult, choice) => {
            const choiceError =
                "Option 'choices' must have a values property (object) and a useKey property (any)"
    ...
    static processFilters(options, state) {
        ...
        const filterStrings = option.reduce((lastResult, filter) => {
            const filterError =
                "Option 'filters' must have a value property (string) and a useIf property (any)"
...
class Validator {
    ...
    static checkType(type, value, mustExist = false, strict = false) {
        ...
        switch (typeof type) {
            ...
            default:
                ERROR.internal(
                    `Validator was given an invalid type to test against: '${type}'`
                )
        }
    }
