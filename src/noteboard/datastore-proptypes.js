...
const NodePropTypes = {
    ...
    listOfNodesOrElements: createRequireablePropType((props, propName, componentName) => {
        return listOfNodesOrElements(props, propName, componentName, propName)
...
function listOfNodesOrElements(list, key, componentName, origPropName) {
    ...
    if (Array.isArray(prop)) {
        for (let i = 0; i < prop.length; i++) {
            const error = listOfNodesOrElements(prop, i, componentName, origPropName)
            if (error) {
                return error
            }
        }
        ...
        if (!isNode && !isElement && !isString && !isNullish) {
            return new Error(`Invalid prop '${origPropName}' supplied to ${componentName}; expected a nested list of (or a single type of) valid React nodes or NodeParents or IDs of such, but instead got '${prop}'`)
        }
    }
    return null
}
